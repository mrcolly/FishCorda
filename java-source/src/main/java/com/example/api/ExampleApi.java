...
public class ExampleApi {
    ...
    @GET
    @Path("prova")
    public Response createIOUProva() throws InterruptedException, ExecutionException {

        final Party otherParty = rpcOps.wellKnownPartyFromX500Name(CordaX500Name.parse("O=Ruler,L=New York,C=US"));
        if (otherParty == null) {
            return Response.status(BAD_REQUEST).entity("Party named cannot be found.\n").build();
        }

        try {
            final SignedTransaction signedTx = rpcOps
                    .startTrackedFlowDynamic(ExampleFlow.Initiator.class, 33, otherParty)
                    .getReturnValue()
                    .get();
